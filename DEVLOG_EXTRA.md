...
| 3.1 | **HIGH** | `interventions.py` (propagate mode) | All notebook interventions run `patch_end_layer=None` with **fixed clean-anchored deltas** — a third protocol that matches neither circuit-tracer's unconstrained *clamp* nor the paper's constrained patching for **multi-layer** feature stacks; module header + docstring actively misdescribe this (docstring claims a default that isn't the default). | **DECISION (2026-07-10, checked against biology.html): adopt (c) — constrained patching.** The paper states "Our interventions in this paper use the 'constrained patching' technique", clamping activations prior to a chosen intervention layer at perturbed values (its dives, incl. multilingual, all run under it; the companion paper's recipe sweeps the end layer). Plan: rerun every notebook intervention (ladders, swap sweeps, polymer, smears) under `patch_end_layer=ℓ` with ℓ swept per experiment via `sweep_patch_end_layer`, make those the headline paper-protocol numbers, keep propagate-mode results as the no-pinning robustness variant, and refresh verdicts where the two disagree (known so far: lookup swap `8` @ 0.715 propagate vs `1` @ 0.732 at ℓ=l_max; polymer flip survives both at 0.747 vs 0.353). Doc fixes from `9d61004` stand. |

...
| 3.1 | **HIGH** | `interventions.py` (propagate mode) | All notebook interventions run `patch_end_layer=None` with **fixed clean-anchored deltas** — a third protocol that matches neither circuit-tracer's unconstrained *clamp* nor the paper's constrained patching for **multi-layer** feature stacks; module header + docstring actively misdescribe this (docstring claims a default that isn't the default). | ✅ **EXECUTED (2026-07-11, fourth A100 session, branch `fix/constrained-patching-pass`): constrained patching is the headline protocol.** Doc fixes from `9d61004` stood; the (c) plan ran end-to-end: (1) plumbing `bc71f40` — `patch_end_layer` passthrough on `steer_and_report`/`paper_swap`/`paper_swap_sweep`/`run_graft` (+ per-job in `run_swap_sweeps`), `choose_patch_end_layer` implementing the ℓ-recipe (sweep [l_max, 35]; suppressions pick `best_end_layer`, injections the new `LayerSweepResult.most_promoting_end_layer`), pinned readout rows dropped (`n_pinned` / `pinned: True`), 6 new tests (116 pass). (2–5) notebook code `cf9356b`, executed `5224c99` — every scoped intervention runs both modes, ℓ stated per result, Summaries rewritten (constrained headline, propagate robustness variant), DEVLOG fourth-session section has the full propagate-vs-constrained tables. Headlines: magnitude/ones dissociation holds at full paper strength for BOTH supernodes (the loose set's propagate breakage was the unpinned always-on cascade); the paper's sum-side smear appears (width 4.10 @ ℓ=35) where propagate flips; polymer lookups-only indirect effect strengthens (`1` @ 0.83, sums 0%); the lookup swap finds NO landing layer (p(`8`) ≤ 0.087 over ℓ=25–35 vs `8` @ 0.715 propagate); multilingual: language-swap null is protocol-complete (p_exp ≈ 0 at every ℓ × strength × direction × format), the operation-swap supernodes reach L35 (single-point sweep, direct-effect only, readouts pinned), the operand swap stops crossing at ℓ = l_max = 28/27 (EN echoes `hot`) — a measured selection-style × protocol interaction. |
...
| 4.2 | MED | verification scope | "Numerically equivalent to circuit-tracer" is established for one config (0.6b / fp32 / CPU / one prompt / default all-mode / one constrained m=−2 steer). Steering parity covers neither `value=` donor injections nor propagate mode (which, per 3.1, is where the semantics diverge). | ✅ **closed (2026-07-11): steering parity extended to every primitive the reproductions use.** `verify_intervention.py` now runs three constrained-mode cases vs circuit-tracer — (A) single-feature m=−2 (the original), (B) a two-layer coupled suppression stack with ℓ above the top steer (clean anchoring, in-range pin, within-range attention response), (C) a `value=` **donor injection** on a clean≈0 recipient feature (the swap protocol's donor half). All PASS at the noise floor (max Δ-diff ≤ 3e-5 on ‖Δ‖ 37–130, cos 1.00000; exits 1 on FAIL). Remaining uncovered by cross-parity: propagate mode only — the deliberate §3.1 divergence, kept as the labeled robustness variant. (Also fixed: the script's ct-side clean-act read indexed `get_activations()[0]` — the logits — so the historical run's `value` basis silently fell back to our clean act; immaterial at ~3e-6 act agreement, now reads the cache correctly.) |
...
- **MED (scope, documented)** The "numerically equivalent to circuit-tracer"
  ...
  their scope.
  **Update (2026-07-11):** steering parity extended — multi-layer constrained stacks
  and `value=` donor injections now cross-verified (3/3 PASS at the noise floor); the
  uncovered surface is propagate mode (deliberate, documented) and multi-token
  generation only.
...
7. ~~Notebook interventions = clean-anchored, fully-propagating decoder-delta
   steering …~~ **Resolved (fourth session, §3.1 executed):** headline notebook
   results now run the paper's constrained patching (ℓ stated per result; the
   cross-verified mode); propagate-mode numbers are kept and labeled as the no-pinning
   robustness variant — cite each as what it is. The swap/9+9 results that exist only
   under propagate are called out as such in both Summaries and the DEVLOG
   fourth-session tables.

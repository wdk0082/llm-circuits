...
$5.25\times$), and fr$\to$en turns the output English --- \emph{big} at $0.753$,
the paper's exact token, crossover $4.5\times$ --- while zh$\to$fr never lands. Past ${\sim}9\times$ both
...
Conventions as in Figure~\ref{fig:swap-operand}; expected curves track the paper's
outcome tokens (for en, \emph{big} rather than the model's own answer \emph{large});
the raw selection has no \emph{say large (zh)} group, so that readout box is absent
in the en$\to$zh panel.}

...
(large\,/\,grand\,/\,\zh{大}); the model's own synonym and hot-antonym answers, which
serve as the swap targets below, are tabulated in Appendix~\ref{app:behaviour}.
...
\section{Model behaviour across formats and languages}\label{app:behaviour}
...
\bottomrule
\end{tabular}
\caption{First generated token per task, chat\,/\,raw format. The paper's expected
behaviours are big\,/\,grand\,/\,\zh{大} (Claude answers ``big''; Qwen3 answers
``large''). The French and Chinese synonym behaviour is degenerate --- the model
echoes the operand (petit\,$\to$\,pet, \zh{小}\,$\to$\,\zh{小}) in both formats ---
so the operation-swap targets in Section~\ref{sec:exp:swaps} are the model's own
answers rather than idealised synonyms.}
\label{tab:behaviour}
\end{table}
